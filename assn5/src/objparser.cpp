...
bool objparser::parsemtl(const std::string& mtlfile, std::map<std::string, material> * materials) {
    ...
    while (std::getline(fh, line)) {
        ...
        if (command == "#" || command == "") {
            ...
            if (matname != "") {  // 插入的是上一个材质
                materials->insert(std::make_pair(matname, mat));
                mat = material();
            }
            ibuff >> matname; // 存当前材质的名字
        }
        else if (command == "Ns") {
            ibuff >> mat.shininess;
        }
...
bool objparser::loadtextures(const std::string& basepath, const std::map<std::string, material>& materials) {
    for (auto it = materials.begin(); it != materials.end(); ++it) {
        ...
        if (mat.diffuse_texture != "") {
            ...
            rgbimage im;
            int nc;
            //stbi_set_flip_vertically_on_load(true); // 试试
            uint8_t* imdata = stbi_load(jpgfile.c_str(), &im.w, &im.h, &nc, 3); // 解析Jpg图像文件的数据 获得宽高以及通道数 nc是解析出来的通道数 3是期待的通道数？
            if (!imdata || nc != 3) {
               printf("Loading texture from %s failed\n", jpgfile.c_str());
               return false;
            }
            im.data.resize(im.w * im.h * nc);
            std::copy(imdata, imdata + im.data.size(), im.data.begin());  // 把从图像文件解析出来的RGB像素 一个个拷贝到rbgimage im的data里面
            // note: this code is not the most efficient.       这种写法不是最高效的 因为它显示拷贝到rbgimage上 又再拷贝了一次rgbimage
            ...
            textures.insert(std::make_pair(mat.diffuse_texture, im)); // 构建纹理名-->rgb图像的映射map
