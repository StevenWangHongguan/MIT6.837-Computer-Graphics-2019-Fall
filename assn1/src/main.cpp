...
GLuint program_color; // 曲线的着色器
// for surfaces, we apply a light+material shader
GLuint program_light;

// These are state variables for the UI UI的状态
bool gMousePressed = false;
enum CurveMode { // 三种状态 啥也没有(只有控制点?) 曲线 以及曲线+法线
   CURVE_MODE_NONE,
   CURVE_MODE_ACTIVE,
...
vector<vector<Vector3f> > gCtrlPoints; // 输入的控制点数据
// vector<Curve> gCurves; // 存储的曲线集合 一系列Curve
vector<string> gCurveNames; //存储的曲线名称集合
...
void drawCurve()
{
    glUseProgram(program_color); //激活曲线着色器
    camera.SetUniforms(program_color);

    glLineWidth(1);
    recorders->curve.draw(GL_LINES); //着色器绘制
...
// 用标准输入流加载物体 也就是运行程序的时候需要用命令行参数来指定swp文件作为加载的物体参数
void loadObjects(int argc, char *argv[])
{
    if (argc < 2) { // 只有一个参数
    ...
    // This does OBJ file output 这一部分处理obj文件的输入
    if (argc > 2) {
...
void recordVertices() {
    ...
    // 为了提高效率 先预先存储到一个数据结构里面 然后再复用这些数据来绘制

    recorders = new Recorders();

    cout << "gCurves.size:" << gCurves.size() << endl;
    ...
    for (int i = 0; i < (int)gCurves.size(); i++) { // 这应该是记录坐标轴吧
...
int main(int argc, char** argv) // 这个就是通过cmd命令执行.exe文件 并传递相应命令行参数 执行的过程
{
