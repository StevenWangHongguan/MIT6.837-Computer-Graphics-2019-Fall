...
void VertexRecorder::record_poscolor(Vector3f pos, Vector3f color) {
	record(pos, Vector3f(0, 0, 0), color); //只记录位置和颜色的话 法线可以置空
}

void VertexRecorder::record(Vector3f pos, Vector3f normal, Vector3f color) // 记录每个顶点的位置 法线和颜色
{
...
/* This implementation uploads data to the GPU on each draw call.
   ...
   data changed. 
*/
void VertexRecorder::draw(GLenum mode) // 绘制方法的实现 核心
{
	if (m_nverts == 0) {
		return;
	}
