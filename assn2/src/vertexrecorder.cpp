...
{
    record(pos, Vector3f(0, 0, 0), Vector3f(1, 1, 1));
}
...
void VertexRecorder::draw(GLenum mode)
{
    ...
    // upload data to GPU
    uint32_t vertexarray; // VAO 顶点数组对象
    ...
    glBufferData(GL_ARRAY_BUFFER, position_nbytes,  // 复制顶点数据到缓存上
        m_position.data(), GL_DYNAMIC_DRAW);

    glEnableVertexAttribArray(0); // 激活顶点属性 指定步长信息
