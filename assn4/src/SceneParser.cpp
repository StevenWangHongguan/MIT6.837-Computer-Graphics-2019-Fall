    ...
    _cubemap(NULL)
{
    ...
    // 通过输入文件名路径找到场景文件 判断格式正确与否 然后解析场景数据
    size_t last_sep = filename.find_last_of("\\/");
    if (last_sep == std::string::npos) { // 这啥意思 表示字符串的末尾
        _basepath = "";
    } else {
        _basepath = filename.substr(0, last_sep + 1);
    }

    std::string ext = filename.substr(filename.size() - 4, 4); // 最后四个字符 拓展名
    if (ext != ".txt") {
        _PostError("ERROR: Wrong file name extension\n");
    }

    _file = fopen(filename.c_str(), "r"); //fopen 参数r 读取文件
...
SceneParser::parseFile() 
{
    ...
    //
    char token[MAX_PARSER_TOKEN_LENGTH]; // 长度为100 表示解析的数据字符最长为100个（其实就是一个const char*类型字符串的长度)
...
SceneParser::parsePerspectiveCamera() 
{
    ...
    getToken(token); assert(!strcmp(token, "}"));
    _camera = new PerspectiveCamera(center, direction, up, angle_radians); // 创建一个新的Perspective相机 存入_camera字段
}
...
SceneParser::parseLights() 
{
    ...
    // read in the objects
    int count = 0;
    while (_num_lights > count) { // 循环读入每个点光源
        getToken(token); 
...
{
    char token[MAX_PARSER_TOKEN_LENGTH];
    char filename[MAX_PARSER_TOKEN_LENGTH]; // ？这个有啥用
    filename[0] = 0;
    Vector3f diffuseColor(1, 1, 1), specularColor(0, 0, 0), transparentColor(0, 0, 0);
    float shininess = 0, refractedIndex = 0;
    getToken(token); assert(!strcmp(token, "{"));
    while (true) {
        getToken(token); 
        ...
        else if (strcmp(token, "refractedIndex") == 0) {
            refractedIndex = readFloat();
        }
    ...
    Material *answer = new Material(diffuseColor, specularColor, shininess, transparentColor, refractedIndex);
...
SceneParser::parseGroup() 
{
    ...
    while (num_objects > count) {
        ...
        if (!strcmp(token, "MaterialIndex")) {
            ...
            _current_material = getMaterial(index); // 根据材质索引获取到材质
        } else {
...
SceneParser::getToken(char token[MAX_PARSER_TOKEN_LENGTH]) 
{
    ...
    assert(_file != NULL);
    int success = fscanf(_file,"%s ",token); // 从_file文件中 将一个个字符串读入token数组
    if (success == EOF) {
        token[0] = '\0';
        return 0;
    }
