...
{
    GLfloat* newColor = new GLfloat[4]; //动态分配内存 若使用静态分配的局部变量 离开函数之后会自动析构 就访问不到里面的元素了
    newColor[0] = (1 - ratio) * color1[0] + ratio * color2[0]; //线性插值
    newColor[1] = (1 - ratio) * color1[1] + ratio * color2[1]; //线性插值
    newColor[2] = (1 - ratio) * color1[2] + ratio * color2[2]; //线性插值
    newColor[3] = (1 - ratio) * color1[3] + ratio * color2[3]; //线性插值
    return newColor;
}
